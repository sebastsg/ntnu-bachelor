...
void game_object_definition_list::save(const std::string& path) const {
	...
	for (auto& definition : definitions) {
		...
		stream.write<int32_t>(definition.script_id.dialogue);
		stream.write<int32_t>(definition.script_id.killed);
...
void game_object_definition_list::load(const std::string& path) {
	...
	for (int32_t i = 0; i < count; i++) {
		...
		definition.script_id.dialogue = stream.read<int32_t>();
		definition.script_id.killed = stream.read<int32_t>();
